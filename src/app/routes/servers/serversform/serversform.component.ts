...
@Component({
  selector: 'app-serversform',
  templateUrl: './serversform.component.html',
  styleUrls: ['./serversform.component.less'],
...
export class ServersformComponent implements OnInit {
  ...
  constructor(private message: NzMessageService, private serversService: ServersService, private fb: FormBuilder, private subject: NzModalSubject) {
  ...
  _submitForm() {
    for (const i in this.validateForm.controls) {
	  ...
	  if (this.validateForm.valid) {
		...
		const serversName = this.validateForm.value.name;
		const age = this.validateForm.value.age;
		// todo: 提交数据到后台
                // this.loginService.login(serversName, password)
                // .then(result => {
